...
class TetrisBoardSpec extends Specification  {
    def "테트리스의 보드는 가로 20칸, 세로 50칸으로 이루어진다."()   {
        when:
        TetrisBoard sut = new TetrisBoard(new BoardCellsControl(), new BlockFactory())
    ...
    def "cell이 다 찬 row가 하나 제거될 때 점수가 10점씩 올라간다."()  {
        ...
        boardCellsControl.updateBoard(_) >> new UpdateBoardResult(true, true, 2, false)
        TetrisBoard sut = new TetrisBoard(boardCellsControl, new BlockFactory())

        given: "테트리스 게임을 시작한다."
        sut.startGame()

        when: "1초 동안 수행하기. 1초 후에 update block 일어나며 각 2개의 row(총 4개)가 삭제된다."
